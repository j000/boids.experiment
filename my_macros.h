 ...
 * there are some issues with comas, so we need to do this like this
...
#define NUM(...)                                                             \
	SELECT_10TH(__VA_ARGS__, TWOORMORE, TWOORMORE, TWOORMORE, TWOORMORE, \
		    TWOORMORE, TWOORMORE, TWOORMORE, TWOORMORE, ONE,         \
		    throwaway)
...
#define ERROR(...)                                                         \
	fprintf(stderr, "[%s:%d:%s()] " FIRST(__VA_ARGS__) "\n", __FILE__, \
		__LINE__, __func__ REST(__VA_ARGS__))
#else
#define ERROR(...)                                                    \
	fprintf(stderr, "[%s:%d] " FIRST(__VA_ARGS__) "\n", __FILE__, \
...
/* skip SDL errors */
#define SDL_PUSH_WARNINGS \
	__pragma("warning(push)") __pragma("warning(disable: 4820)")
