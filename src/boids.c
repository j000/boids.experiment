...
// declaraions
// TODO(j000): move to *.h
global_variable pListItem head;
void cleaningatexit(void);
void bail(const char*, const char*) NORETURN;

// globals
global_variable pListItem head = NULL;

void cleaningatexit(void) { cleaning(&head); }

// prints error and exits application
// TODO(j000): show warning window
...
int main(/* int argc, char *argv[], char **env_var_ptr */)
{
	...
	if (SDL_Init(SDL_INIT_VIDEO | SDL_INIT_TIMER) != 0) {
		bail("Unable to initialize SDL2", SDL_GetError());
	}
	atexit(SDL_Quit);
	...
	if ((window = SDL_CreateWindow("Hello World!", SDL_WINDOWPOS_CENTERED,
...
// loop flag
	...
	while (keepgoing) {
		...
		// FIXME(j000): That's not how you do fixed framerate
		SDL_Delay(17);
#ifdef DEBUG
		--keepgoing;
#endif
